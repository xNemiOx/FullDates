...
const buttons = document.querySelectorAll('.btn__nav-button');
const buttonText = document.querySelectorAll('.btn__text');
...
function clickHandler(e) {
    ...
    // numberPage.innerHTML = `${(+index + 1).toString().padStart(2, '0')}/04`
    ...
    const targetRotate = +index*(360/buttons.length)

    buttonNav.style.transform = `rotate(${-targetRotate}deg)`
    buttonText.forEach((text)=>text.style.transform = `rotate(${targetRotate}deg)`)
